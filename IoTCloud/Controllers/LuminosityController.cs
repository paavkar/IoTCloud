...
namespace IoTCloud.Controllers
{
    ...
    public class LuminosityController(IReadingsService readingsService, IUserService userService, ISensorsService sensorsService) : ControllerBase
    {
        ...
        public async Task<IActionResult> AddReading(string apiKey, string sensorName, float luminosity)
        {
            ...
            var isOperationSuccessful = await readingsService.AddLuminosityReading(luminosity, sensorName, existingKey.UserId, DateTimeOffset.UtcNow);
        ...
        public async Task<IActionResult> AddReading(string apiKey, string sensorName, int binary)
        {
            ...
            var isOperationSuccessful = await readingsService.AddBinaryReading(binary, sensorName.ToLower(), existingKey.UserId, DateTimeOffset.UtcNow, ReadingType.Luminosity);
